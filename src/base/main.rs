...
fn print_usage() {
    eprintln!("usage:");
    eprintln!("  cargo run --bin base -- --single \"the cat sat on the table\" --hits 1000");
    eprintln!("  cargo run --bin base -- --disaggregated \"the cat sat on the table\" --hits 1000");
    eprintln!(
        "  cargo run --bin base -- bench-part2-query --iterations 100000 --pods 64 --blocks 32 --dropoffs 4"
    );
    eprintln!(
        "  cargo run --bin base -- bench-part2-compare --iterations 100000 --pods 64 --blocks 32 --dropoffs 4"
    );
    eprintln!(
        "  cargo run --bin base -- bench-part2-repr-compare --iterations 100000 --pods 64 --blocks 32 --dropoffs 4"
    );
    eprintln!("  cargo run --bin base -- bench-part2-shards --chains 10000 --blocks-per-chain 64");
    eprintln!(
        "  cargo run --bin base -- bench-part2-concurrency --readers 8 --writers 2 --duration-secs 10 --pods 64 --blocks 32"
    );
    eprintln!();
    eprintln!("if running the compiled binary directly:");
    eprintln!("  target/debug/base --single \"prompt\" --hits 1000");
}
